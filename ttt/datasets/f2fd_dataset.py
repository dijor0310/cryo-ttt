...
class F2FD_Dataset(Dataset):
    ...
    def fourier_masking(self, patch):
        ...
        # Inverse FFT
        ifft_patch = np.fft.irfftn(
            np.fft.ifftshift(fft_patch, axes=(-3, -2)), s=patch.shape
        )
    ...
    def __getitem__(self, idx):
        ...
        gt_membrane = self.pad(
            gt_membrane, (self.patch_size, self.patch_size, self.patch_size)
        )
...
class F2FD_DatasetV2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "raw_subtomo": torch.Tensor(sample["subtomo"])
            .unsqueeze(0)
            .to(torch.float32),
