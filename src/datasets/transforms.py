...
class F2FDMaskingTransform(Transform):
    ...
    def __call__(self, patch: Dict):
        ...
        # Inverse FFT
        ifft_patch = torch.fft.irfftn(torch.fft.ifftshift(masked_fft_patch, dim=(-3, -2)), dim=(-3, -2, -1), s=patch.shape[2:])
    ...
    def mask(self, fft_patch):
        ...
        # print(mask_shape)
        patch_mask = (np.random.rand(*mask_shape) > self.bernoulli_mask_ratio).astype(np.float32)
        ...
        bernoulli_mask = torch.tensor(bernoulli_mask, dtype=torch.float, device=fft_patch.device)

        # Random Phase Inversion
        phase_flip = (np.random.rand(*fft_patch.real.shape) < self.phase_inversion_ratio).astype(np.float32)
        phase_flip = torch.tensor(phase_flip, dtype=torch.float, device=fft_patch.device)
        fft_patch_inverted = fft_patch * ((-1) ** phase_flip)
        
        # Spherical Mask to keep low frequencies
        center = size // 2
        radius = np.random.randint(int(self.min_mask_radius * size), int(self.max_mask_radius * size))
        x, y, z = np.meshgrid(np.arange(size), np.arange(size), np.arange(size // 2 + 1), indexing='ij')
        ...
        return fft_patch_inverted * overall_mask[None, None, ...].repeat(fft_patch.shape[0], 1, 1, 1, 1)
