...
class CryoETMemSegDataset(Dataset):
    ...
    def __init__(
        ...
        img_folder: str = config.method.img_folder
        label_folder: str = config.method.label_folder
        train: bool = not is_validation
        aug_prob_to_one: bool = config.method.aug_prob_to_one
        patch_size: int = config.method.patch_size
        on_the_fly_loading: bool = config.method.on_the_fly_loading
    ...
    def __getitem__(self, idx: int) -> Dict[str, np.ndarray]:
        ...
        # print(idx_dict["image"].shape, idx_dict["label"].shape)
        return idx_dict
    ...
    def get_random_crop(self, idx_dict: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        if x <= self.patch_size or y <= self.patch_size or z <= self.patch_size:
            ...
            # assert (
            #     label.shape[1] == self.patch_size
            #     and label.shape[2] == self.patch_size
            #     and label.shape[3] == self.patch_size
            # ), f"Label shape is {label.shape} instead of {self.patch_size}"
        ...
        z_start = np.random.randint(0, z - z_crop + 1)
        # x_start = 400
        # y_start = 400
        # z_start = 50
        img = img[
    ...
    def load_data_sample(self, idx: int) -> Dict[str, np.ndarray]:
        ...
        label = mrcfile.read(self.data_paths[idx][1])
        img = mrcfile.read(self.data_paths[idx][0])
