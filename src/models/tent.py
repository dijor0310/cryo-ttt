...
#     print(x.shape)
#     x = torch.flatten(x).unsqueeze(1)
#     a = x.sigmoid() * nn.LogSigmoid()(x)
#     b = (1 - x.sigmoid()) * ((1 - x.sigmoid()).log())
#     return -(a + b)
def softmax_entropy(logits):
    # Convert logits to probabilities using the sigmoid function
    logits = torch.flatten(logits).unsqueeze(1)
    probabilities = torch.sigmoid(logits)

    # Clamp the probabilities to avoid log(0) or log(1), which can cause numerical issues
